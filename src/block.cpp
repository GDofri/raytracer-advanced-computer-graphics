...
ImageBlock::ImageBlock(const Vector2i &size, const ReconstructionFilter *filter) 
    ...
    // Initialize splat image
    m_splat.resize(m_size.y(), m_size.x());
...
void ImageBlock::put(ImageBlock &b) {
    ...
    m_splat.block(b.getOffset().y(), b.getOffset().x(), b.getSize().y(), b.getSize().x())
        += b.m_splat.topLeftCorner(b.getSize().y(), b.getSize().x());
...
void ImageBlock::splat(const Point2f &_pos, const Color3f &value)
{
    if (!value.isValid()) {
        /* If this happens, go fix your code instead of removing this warning ;) */
        cerr << "Integrator-splat: computed an invalid radiance value: " << value.toString() << endl;
        return;
    }

    /* Convert to pixel coordinates within the image block */
    Point2f pos(
            _pos.x() - 0.5f - (m_offset.x()),
            _pos.y() - 0.5f - (m_offset.y())
    );

    // Check if pixel is within the image block
    if ((int)pos.x() < 0 || (int)pos.x() >= m_size.x() || (int)pos.y() < 0 || (int)pos.y() >= m_size.y())
    {
        std::cerr << "Splat position is outside of the image block. Pos: " + pos.toString() + " Size: " + m_size.toString() << std::endl;
        return;
    }
    m_splat.coeffRef((int) pos.y(), (int) pos.x()) += Color3f(value);
}
