...
public:
    ...
    virtual Color3f sampleWi(const nori::Intersection &its, Point2f &rasterLoc, float &pdf) const = 0;

    virtual void pdf(const Ray3f &ray, float &pdfPos, float &pdfDir) const = 0;
    ...
    /// Return the camera's world location
    Point3f getWorldPosition() const { return m_cameraWorldLoc; }

    /// Return the camera's normal
    Normal3f getCameraNormal() const { return m_cameraNormal; }

protected:
    Vector2i m_outputSize;
    Point3f m_cameraWorldLoc;
    Normal3f m_cameraNormal;
