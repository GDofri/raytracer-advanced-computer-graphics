...
public:
    ...
    virtual void render(const Scene *scene, const std::string &fileName, int threadCount, bool showGUI) const = 0;
...
public:
    ...
    void render(const Scene *scene, const std::string &fileName, int threadCount, bool showGUI) const override;


    virtual void preprocess(const Scene *scene) const override{ } ;
...
private:
    void renderBlock(const Scene *scene, Sampler *sampler, ImageBlock &block) const;
};
