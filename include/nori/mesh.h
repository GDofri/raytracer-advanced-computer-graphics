...
public:
    ...
    /**
     * \brief Sample a random point on the surface of the mesh
     * @param sampler
     * @return A tuple containing the sampled point, the normal at the point,
     * the PDF of the sample and the index of the face
     */
    std::tuple<Vector3f, Vector3f, float> sample(Sampler &sampler) const;


    /**
     * * \brief Get the pdf of sampling a point on the surface of the mesh
     * @return pdf
     */
    float pdf() const;

    /**
     * \brief Return the vertices of the face with the given index
     * @param faceIdx
     * @return A tuple containing the vertices of the face
     */
    std::tuple<Point3f, Point3f, Point3f> getFaceVertices(uint32_t faceIdx) const;

    /**
     * \brief Return the normals of the face with the given index
     * @param faceIdx
     * @return A tuple containing the normals of the face
     */
    std::tuple<Vector3f, Vector3f, Vector3f> getFaceNormals(uint32_t faceIdx) const;
...
protected:
    ...
    Mesh();
protected:
    std::string m_fileName;              ///< Identifying name
    std::string m_name;                  ///< Name of the mesh
    MatrixXf      m_V;                   ///< Vertex positions
    MatrixXf      m_N;                   ///< Vertex normals
    MatrixXf      m_UV;                  ///< Vertex texture coordinates
    MatrixXu      m_F;                   ///< Faces
    DiscretePDF   m_facePDF;             ///< PDF to randomly sample faces by their area.
