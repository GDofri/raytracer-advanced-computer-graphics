...
public:
    ...
    /**
     * \brief Sample an emitter from the scene.
     *
     * \param sample
     *    A uniformly distributed random sample in the interval [0,1)
     *
     * \return
     *    A pair consisting of a pointer to the emitter and the
     *    probability density of the sample
     */
    std::pair<const Mesh *, float >sampleEmitter(float u) const;

    float emitterSamplePdf() const;
